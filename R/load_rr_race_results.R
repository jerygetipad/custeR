#' Load RR Race Results
#'
#' Loads the Racing Reference 'Race Results' for the following series:
...
#' @returns data.frame of racing-reference's 'race results' page for all races of a given year
#'
#' @export
#' @examples
#' load_rr_race_results(year=2022, series="cup")
#'

load_rr_race_results <- function(year, series, time_pause=1) {
  ...
  year_valid <-
    switch(
      ...
      nascar_busch = year %in% c(1984:2023),
      nascar_convertible = year %in% c(1956:1959),
      nascar_cup = year %in% c(1949:2023),
      ...
      nascar_modified = year %in% c(1979:2023),
      ...
      nascar_trucks = year %in% c(1995:2023),
  ...
  url = glue::glue( "https://raw.githubusercontent.com/armstjc/racing-data-repository/main/racing_reference/{series}/race_results/{year}_race_results.csv")
